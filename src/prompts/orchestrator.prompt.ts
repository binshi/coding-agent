...
1. Use the code-quality-analyzer agent with this prompt: "Analyze these files from PR #${prNumber} in ${owner}/${repo} for code quality issues: [list all changed file paths]. For each file, return a JSON object with file, issues array, overallScore (0-100), and summary. Return a JSON array of results, one per file."

2. Use the test-coverage-analyzer agent with this prompt: "Analyze test coverage for these files from PR #${prNumber} in ${owner}/${repo}: [list all changed file paths]. For each file, return a JSON object with file, hasTests, testFiles, untestedPaths array, coverageEstimate (0-100), and summary. Return a JSON array of results, one per file."

3. Use the refactoring-suggester agent with this prompt: "Suggest refactorings for these files from PR #${prNumber} in ${owner}/${repo}: [list all changed file paths]. For each file, return a JSON object with file, suggestions array, and summary. Return a JSON array of results, one per file."

IMPORTANT: Pass ALL file paths to each agent in a single call. Do NOT call each agent per-file.
